package com.nikhil.yt.discord

import com.my.kizzy.gateway.entities.op.OpCode
import org.json.JSONObject

typealias GatewayOp = OpCode

/** Snapshot of an active gateway session, kept around so a dropped connection can RESUME. */
data class GatewaySessionState(
    val sessionId: String?,
    val seq: Int = 0,
    val resumeGatewayUrl: String? = null,
)

data class GatewayReadyEvent(
    val sessionId: String,
    val resumeGatewayUrl: String,
    val user: User,
) {
    data class User(
        val id: String = "",
        val username: String = "",
    )

    companion object {
        fun fromJson(obj: JSONObject): GatewayReadyEvent {
            val userObj = obj.optJSONObject("user")
            return GatewayReadyEvent(
                sessionId = obj.optString("session_id"),
                resumeGatewayUrl = obj.optString("resume_gateway_url"),
                user = User(
                    id = userObj?.optString("id").orEmpty(),
                    username = userObj?.optString("username").orEmpty(),
                ),
            )
        }
    }
}

data class GatewayCloseInfo(
    val code: Int,
    val reason: String = "",
    val resumable: Boolean = true,
    val session: GatewaySessionState? = null,
)

object GatewayDefaults {
    const val GATEWAY_URL = "wss://gateway.discord.gg"
    const val GATEWAY_VERSION = 10
    const val HELLO_TIMEOUT_MS = 10_000L
    const val HEARTBEAT_INTERVAL = 41_250L
    const val IDENTIFY_INTERVAL = 5_000L
}

object GatewayCapabilitiesFlags {
    const val MESSAGE_CONTENT_V2 = (1 shl 15)
    val FLAGS = mapOf("MESSAGE_CONTENT_V2" to MESSAGE_CONTENT_V2)
}

object IntentsFlags {
    const val GUILD_MESSAGES = (1 shl 9)
    const val MESSAGE_CONTENT = (1 shl 15)
    val FLAGS = mapOf(
        "GUILD_MESSAGES" to GUILD_MESSAGES,
        "MESSAGE_CONTENT" to MESSAGE_CONTENT,
    )
}

val NON_RESUMABLE_CLOSE_CODES = setOf(4004, 4010, 4011, 4012, 4013, 4014)
const val DISCORD_APPLICATION_ID = "1284533208369975316"
const val DISCORD_APPLICATION_ID_LONG: Long = 1284533208369975316L
const val DISCORD_REDIRECT_SCHEME = "velune"
